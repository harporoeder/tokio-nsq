    ...
    pub fn new(inner: S, level: u8) -> Self {
        Self {
            ...
            encoder: DeflateEncoder::with_quality(
                inner,
                async_compression::Level::Precise(level as u32),
            ),
        }
    }
}
    ...
    fn poll_write(
        ...
        loop {
            if self.written == buf.len() {
                match AsyncWrite::poll_flush(Pin::new(&mut self.encoder), cx) {
                    Poll::Ready(Ok(())) => {
                        self.written = 0;
            ...
            match AsyncWrite::poll_write(Pin::new(&mut self.encoder), cx, buf) {
                Poll::Ready(Ok(0)) => {
